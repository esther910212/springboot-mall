...
public class UserControllerTest {
    ...
    @Autowired
    private UserDao userDao; // 可以自由的去注入的 bean
    ...
    public void register_success() throws Exception {
        ...
        //先去創建一個 UserRegisterRequest 的請求參數 然後在裡面 去填上 email 還有密碼的值 去模擬前端的輸入

        String json = objectMapper.writeValueAsString(userRegisterRequest);
        //將這些參數 去使用 objectMapper 去轉換成是一個 json 的字串

        RequestBuilder requestBuilder = MockMvcRequestBuilders
                .post("/users/register")
                .contentType(MediaType.APPLICATION_JSON)
                .content(json);
        //用 mockMvc 的功能 去 call 這個 api
    ...
    private void register(UserRegisterRequest userRegisterRequest) throws Exception { //重複性很高的程式去包裝成是一個方法
        String json = objectMapper.writeValueAsString(userRegisterRequest);
